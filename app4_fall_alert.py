	    ...
	    # boxes, idxs, idArray
		    ...
		    # extract the bounding box coordinates
			    ...
			    # draw a bounding box rectangle and label on the frame
                    ...
                    send_email_with_attachment(username="saimadhav.alert@gmail.com",password="m@keskilled",receiver="parvathanenimadhu@gmail.com",subject="Fall Alert",body="This alert is generated automatically",imagefile="fall/demo.png")
                    print("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
                else:
                    color = [int(c) for c in COLORS[classIDs[i]]]
